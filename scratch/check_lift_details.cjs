...
async function checkData() {
  console.log("Checking Mismatch details for LF-142 and LF-148...");
  const { data, error } = await supabase
    .from('Mismatch')
    .select('"Lift Number", "Firm Name", "Transporter Name", Planned2, Actual2')
    .or('"Lift Number".eq.LF-142,"Lift ID".eq.LF-142,"Lift Number".eq.LF-148,"Lift ID".eq.LF-148');

  if (error) console.error(error);
  else console.log("Mismatch details:", JSON.stringify(data, null, 2));
}
