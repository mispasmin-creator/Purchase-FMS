...
export default function FullkittingTransportingPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [
                ...
                supabase.from("LIFT-ACCOUNTS").select('"id","Firm Name","Lift No","Vendor Name","Raw Material Name","Indent no.","Actual 3","Timestamp","Qty","Lifting Qty","Total Bill Quantity","Area lifting","Bill No.","Rate","Truck Qty","Bill Image","Driver No.","Lead Time To Reach Factory (days)","Bilty Image","Bilty No.","Type Of Transporting Rate","Transporter Rate","Truck No.","Date Of Receiving","Type","Image Of Weight Slip","Moisture","Actual Quantity"').order("Timestamp", { ascending: false }),
                supabase.from("fullkittin").select('"id","Lift No","Bilty Number","Material Load Details","Transporter Name","Bilty Image","Transporter Bill Image","Fullkitting Remarks","Rate Type","Amount","Vehicle Number"'),
                supabase.from("INDENT-PO").select('"Indent Id.","Timestamp","Firm Name","Generated By","Quantity","Priority","Delivery Order No.","Notes","Approved Qty","Approval Status","Remarks","Have To Make PO","Total Quantity","Total Amount","PO Copy","Advance To Be Paid","To Be Paid Amount","PO Notes"'),
                supabase.from("Mismatch").select('"Actual6","Bilty No.","Transporter Name","Transporter","Type Of Rate","Total Freight","Lift Number"'),
    ...
    useEffect(() => {
        pendingPagination.setTotalRows(pendingKitting.length);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [pendingKitting.length]);

    useEffect(() => {
        historyPagination.setTotalRows(historyKitting.length);
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [historyKitting.length]);
    ...
    const renderTableSection = (tabKey, title, description, data, columnsMeta, visibilityState, isLoading, hasError, emptyMessage, pagination) => {
        ...
        // The header count and "select all" (which selects across every
        // loaded row, not just this page) both keep using the full `data` —
        // only the rendered rows are paged.
                ...
                <CardContent className="p-4">
                    ...
                    <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col">
                        ...
                        <TabsContent value="pending" className="flex-1 flex flex-col mt-0">
                            {renderTableSection("pending", "Pending Kitting", "Entries where Planned (W) is filled but Actual (X) is empty.", pendingKitting, pendingKittingColumns, visiblePendingColumns, loading, !!error, "No entries are currently pending kitting.", pendingPagination)}
                        </TabsContent>
                        <TabsContent value="history" className="flex-1 flex flex-col mt-0">
                            {renderTableSection("history", "Kitting History", "Entries where both Planned (W) and Actual (X) are filled.", historyKitting, historyKittingColumns, visibleHistoryColumns, loading, !!error, "There is no kitting history to show.", historyPagination)}
                        </TabsContent>
                    </Tabs>
                </CardContent>
            </Card>
