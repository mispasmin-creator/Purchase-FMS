...
const submitFormData = async () => {
  ...
  try {
    ...
    // Create form data object - always include actual for Column V
    ...
    // Only add delay if status is not "Done" (for Column W)
    if (data.status !== "Done") {
...
const fetchData = async () => {
  try {
    ...
    let parsedData = data.table.rows.map((row, index) => {
      ...
      // Check if column V (index 21) has data (Actual column)
      ...
      const rowData = {
        id: index,
        timestamp: formatDate(getCellValue(row, 0)) || '', // Column A (index 0)
        liftNumber: getCellValue(row, 1) || '',           // Column B (index 1)
        type: getCellValue(row, 2) || '',                // Column C (index 2)
        billNo: getCellValue(row, 3) || '',              // Column D (index 3)
        partyName: getCellValue(row, 4) || '',           // Column E (index 4)
        productName: getCellValue(row, 5) || '',         // Column F (index 5)
        qty: getCellValue(row, 6) || '',                 // Column G (index 6)
        areaLifting: getCellValue(row, 7) || '',         // Column H (index 7)
        truckNo: getCellValue(row, 8) || '',             // Column I (index 8)
        transporterName: getCellValue(row, 9) || '',     // Column J (index 9)
        billImage: getCellValue(row, 10) || '',          // Column K (index 10)
        biltyNo: getCellValue(row, 11) || '',            // Column L (index 11)
        typeOfRate: getCellValue(row, 12) || '',         // Column M (index 12)
        rate: getCellValue(row, 13) || '',               // Column N (index 13)
        truckQty: getCellValue(row, 14) || '',           // Column O (index 14)
        biltyImage: getCellValue(row, 15) || '',         // Column P (index 15)
        qtyDifferenceStatus: getCellValue(row, 16) || '', // Column Q (index 16)
        differenceQty: getCellValue(row, 17) || '',      // Column R (index 17)
        weightSlip: getCellValue(row, 18) || '',         // Column S (index 18)
        totalFreight: getCellValue(row, 19) || '',       // Column T (index 19)
        status: getCellValue(row, 23) || '',             // Column X (index 23)
        remarks: getCellValue(row, 24) || '',            // Column Y (index 24)
      };
...
const toggleColumnFilter = () => {
  ...
  <tr>
    ...
    {visibleColumns.qtyDifferenceStatus && <th className="px-6 py-4 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Quantity Difference Status</th>}
