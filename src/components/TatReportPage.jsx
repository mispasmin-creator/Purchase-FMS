  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [
        ...
        supabase
          .from("LIFT-ACCOUNTS")
          .select('id, "Lift No", "Indent no.", "Bilty No.", Timestamp, "Date Of Receiving", "Date Of Test", "Actual 3"')
          .order("Timestamp", { ascending: false }),
        supabase
          .from("INDENT-PO")
          .select('id, "Indent Id.", po_number, "Vendor name", Vendor, Material, "Firm Name", Priority, Timestamp, Actual2, Planned9, Actual9, Actual3, "Generated By", "Type Of Indent", "Approved Qty", Quantity, UOM')
          .order("Timestamp", { ascending: false }),
        supabase
          .from("fullkittin")
          .select('id, "Lift No", "Bilty Number", "Indent No", Timestamp'),
        supabase
          .from("Mismatch")
          .select('id, "Lift Number", "Lift ID", Actual2')
      ]);
