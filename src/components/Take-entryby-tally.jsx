...
const submitFormData = async () => {
    ...
    try {
      ...
      // Create form data without Column AL (delay field)
  ...
  // Check if column AJ (index 35) has data AND column AK (index 36) is empty/null
  const ajValue = getCellValue(row, 35); // Column AJ
  const akValue = getCellValue(row, 36); // Column AK
  
  // Only include rows where AJ has data AND AK is empty
  ...
  const rowData = {
    ...
    status: getCellValue(row, 38) || '', // Column AM (index 38)
    remarks: getCellValue(row, 39) || '' // Column AN (index 39)
  };
