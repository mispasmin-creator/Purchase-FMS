...
export default function StockApproval() {
      ...
      try {
       ...
       const rowsData = data.table.rows.map((row, rowIndexInSheetData) => {
          const rowObject = {
            ...
            _rowIndex: rowIndexInSheetData + dataStartRow , // Subtract 1 to target the row before
            _actualSheetRow: rowIndexInSheetData + dataStartRow , // Store actual sheet row (one before)
  ...
  // CRITICAL FIX: Use pendingIndents which has the correct row data
  const selectedPendingIndents = pendingIndents.filter(indent => selectedRows[indent.id]);

  console.log("=== DEBUG: Row index verification ===");
  console.log("Selected pending indents:", selectedPendingIndents.map(ind => ({
    id: ind.id,
    _rowIndex: ind._rowIndex,
    rawRowIndex: ind.rawRowIndex,
    columnL_Check: ind.columnL_Check,
    columnM_Check: ind.columnM_Check
  })));

  for (const indent of selectedPendingIndents) {
    if (!indent._rowIndex) {
      errorMessages.push(`Could not find row index for indent ${indent.id}.`);
      continue;
    }
    ...
    try {
      ...
      // Use correct column indices - based on your screenshot structure:
      // Column M = 13 (Approval Time/Actual1)
      // Column O = 15 (Approved Qty)
      // Column P = 16 (Approval Status)
      // Column Q = 17 (Remarks)
      const cellUpdates = {
        [`col13`]: timestamp,                       // Column M: Actual1/Approval Time
        [`col15`]: approvedQtys[indent.id] || indent.quantity, // Column O: Approved Qty
        [`col16`]: approvalStatuses[indent.id],     // Column P: Approval Status
        [`col17`]: finalRemark                      // Column Q: Remarks
      };
      
      console.log(`Submitting for indent ${indent.id}:`, {
        rowIndex: indent._rowIndex,
        actualSheetRow: indent._rowIndex, // This should be the actual Google Sheets row
        indentId: indent.id,
        cellUpdates: cellUpdates
      });
      
      // IMPORTANT: Verify we're using the correct row index
      // The _rowIndex should be the actual Google Sheets row number
      const payload = new URLSearchParams({
        sheetName: sheetName,
        action: "updateCells",
        rowIndex: indent._rowIndex.toString(), // This should be the correct sheet row
      ...
      if (responseData && (responseData.status === 'error' || responseData.success === false)) {
        throw new Error(responseData.message || 'Script reported failure.');
      }
      
      successCount++;
      console.log(`✓ Updated ${indent.id} at sheet row ${indent._rowIndex}`);
  ...
  const getHistoryTableColumns = useMemo(() => {
    const explicitHistoryColumns = [
      ...
      { dataKey: "col42", header: "Vendor Name" },
