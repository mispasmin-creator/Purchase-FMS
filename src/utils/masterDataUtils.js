...
// Several pages (Indent, Fullkitting, Lift, Arrange Logistics, Three Party)
// each call fetchMasterData() on their own mount. Master rarely changes
// within a session, so cache the resolved result for a few minutes and
// share one in-flight request across near-simultaneous callers instead of
// re-scanning the whole table every time any of those pages is opened.
let masterDataCache = null;
let masterDataCacheTime = 0;
let masterDataFetchPromise = null;
...
export async function fetchMasterData() {
    const now = Date.now();
    if (masterDataCache && (now - masterDataCacheTime) < MASTER_DATA_CACHE_TTL_MS) {
        return masterDataCache;
    }
    if (masterDataFetchPromise) {
        return masterDataFetchPromise;
    }
    masterDataFetchPromise = fetchMasterDataUncached()
        .then((result) => {
        ...
        .finally(() => {
            masterDataFetchPromise = null;
        });
    return masterDataFetchPromise;
}

async function fetchMasterDataUncached() {
    try {
        let data = [];
        let from = 0;
        let hasMore = true;
        ...
        while (hasMore) {
            const { data: chunk, error } = await supabase
                .from("Master")
                .select('"Generated By","Vendor Name","Raw Material Name","Type Of Indent","Rate Type","Type Of Rate","Area Lifting","Type","Transporter Name","Transporter Name 2","Firm Name","Type Of KYC Form","Vendor Name KYC","Product Name","GST Number","Current Bank A.C. No","IFSC Code","Phone Number","Email","UOM"')
                .range(from, from + 999);
